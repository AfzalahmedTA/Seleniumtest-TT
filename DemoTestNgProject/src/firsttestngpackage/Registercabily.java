...
public class Registercabily {
	...
	HSSFCell cell;
	WebDriverWait wait;
	File  src=new File("D:\\Afsal\\Registercabily.xls");
  ...
  public void Testcase2() throws IOException,Exception{
	  ...
	  for(int i=3;i<=4;i++) {
		 ...
		 String mess="Please enter only alphabets";
		  WebElement message2=driver.findElement(By.xpath("//div[4]//label[1]"));   
		  if(message2.getText().equalsIgnoreCase(mess)) {
			  System.out.println("the ok");
			  sheet.getRow(3).createCell(3).setCellValue(mess);
		 } 
		 
		  else {
			 WebElement Errormsg=wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("mailErr")));
			 String ok=Errormsg.getText();
          sheet.getRow(4).createCell(3).setCellValue(ok);
